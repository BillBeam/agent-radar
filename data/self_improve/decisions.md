...
- **教训入册**：① 生产管线不能骑 PATH 上「今天恰好是谁」的二进制——brew 升级=不受控变更面；
  ② CLI 的失败诊断在 stdout（json envelope），只读 stderr 会盲；③ 二进制 `strings` 是环境
  变量真名的唯一权威（二手博客给的名字少个 `BYTE_` 白烧一轮）；④ 升级回归的止损点=回滚已证
  好版本，考古（1M beta/非流式回落的确切触发条件）留给 upstream issue，不挡生产恢复。
