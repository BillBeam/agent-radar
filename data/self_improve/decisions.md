...
## 07-08 下午 · deepread 9/9 全灭事件：CLI 2.1.204 请求形态回归（当日破案+回滚固定）

- **事件**：07-08 中午重跑（286 池、10 入选）deepread 9 篇新条目全灭，只有 checkpoint 复用的
  1 篇幸存；页面 9 篇简介裸奔。错误全是 `exit 1: ` 空 stderr、每篇烧穿 3 次重试。
- **破案链（每步实锤）**：① wrapper 失败路径只留 stderr 丢 stdout → 盲 90 分钟；补「stderr
  空则取 stdout」后第一发就现形 `API Error: Connection closed mid-response`（304s）。
  ② 该字符串就在 CLI 二进制里（strings 抓到）= **CLI 字节看门狗自己掐的**。③ 定量：成功调用
  287s/今早 10 篇全 ≤238s，失败全压 303–306s ⇒ 300s 铁口；`ttft_ms/ttft_stream_ms: None` =
  **服务器 300+ 秒零字节、SSE 都没开**。④ 排除法：额度（pong/散文 30K 字符全过）、代理
  （剥环境代理同死；错误源在 CLI 内）、看门狗调参（`CLAUDE_BYTE_STREAM_IDLE_TIMEOUT_MS`+
  `API_TIMEOUT_MS` 拉满 20min 只把死点推到 606/909s——CLI 内部重试次数的随机表现，服务器
  始终静默）、强制流式（`CLAUDE_CODE_DISABLE_NONSTREAMING_FALLBACK=1` 同死）、显式模型 ID
  （`claude-opus-4-8` 同死）。⑤ **时间线钉死版本**：brew 于 07-08 11:14 升 CLI → 2.1.204
  （软链 mtime），今早 08:42–11:02 旧版同尺寸 payload（73K grounding）全部秒开流；11:34 起
  新版全灭。修复信封另显示新版给 opus 挂 1M 上下文（contextWindow=1000000，小 payload 也挂
  但能快回）——大 payload（~36K token）在该路径整段静默排队 >300s。
- **修（三层，pytest 247 绿）**：① `claude_code._run` 失败诊断补 stdout（根治「exit 1 空白」
  盲区）+ `Connection closed mid-response`/`internal server` 进瞬态重试分类；② 看门狗/请求
  超时 env 与 wrapper 调用超时对齐（`CLAUDE_BYTE_STREAM_IDLE_TIMEOUT_MS`/`API_TIMEOUT_MS`
  = timeout*1000，真名从二进制 strings 验出——文档/博客流传的 `CLAUDE_STREAM_IDLE_TIMEOUT_MS`
  是假名）+ `DISABLE_AUTOUPDATER=1`；③ **管线 CLI 固定版本**：`AGENT_RADAR_CLAUDE_BIN`
  （.env，机器本地）指向 native 安装的 **2.1.201 versioned 二进制**（`claude.ai/install.sh
  -s 2.1.201`，不占 PATH、不影响他交互用的 brew 2.1.204），wrapper 优先取之。**判决实验**：
  同 payload 2.1.204 十二连死 vs 2.1.201 一次通过 300s 线。
- **⚠️ 根因修正（提交后 10 分钟被自己证伪——诚实入册）**：pinned 2.1.201 精确复刻**同样失败**
  （606s、同报错、同零字节、同样 cw=1M）→ **「CLI 版本回归」理论死亡**（哨兵只看「活过 300s」
  就报捷=内部重试两轮的假象；commit 20eb300 的判决词错了，以本条为准）。重排全部事实后唯一
  幸存理论：**账户级大请求静默限流**——今早新鲜窗 10 篇全成（≤238s）；两天都是中午烧掉
  ~20-25 万输出 token 后大 payload（~25K+ token 入参）开始永久静默（**07-07 12:42 eval 七连
  exit 1 = 同模式前科**），小调用全程无恙；新旧 CLI 同死/剥代理同死/一切客户端调参无效=服务
  器端行为；~300s 一掐=某层静默连接回收。**处置**：等 5h 窗滚动重置后 `--only 1` 探针验证再
  regen；三层修复仍全部成立（可见性/重试分类/版本固定都是正确加固，且本次破案依赖 ①）。
- **教训入册**：① 生产管线不能骑 PATH 上「今天恰好是谁」的二进制——brew 升级=不受控变更面
  （固定版本这层照留）；② CLI 的失败诊断在 stdout（json envelope），只读 stderr 会盲；
  ③ 二进制 `strings` 是环境变量真名的唯一权威（二手博客给的名字少个 `BYTE_` 白烧一轮）；
  ④ **时间混杂变量是根因分析头号陷阱**：版本升级(11:14)与用量累积(11:52 爆发)几乎同刻，
  「今早旧版好好的」同时支持两个理论——差点在错误根因上收案，靠「回滚后复测」才翻案；
  ⑤ 哨兵/监控的成功判据要盯**终态**（完成+内容），不是中间里程碑（活过 300s）。
- **第二轮收口（用户令「确保不再出现类似问题」）**：审计发现 `_health_line` 当天其实**有**报
  「27/28 源成功，失败：arxiv-agents」——缺的不是披露、是**把点连起来**（源挂 ↛ 为什么只有
  9 篇）。补三块：① 失败行追加「缺口不丢，该源恢复后自动补课」（安抚=B2 真行为）；② **薄推
  送自解释行**——`入选 N/cap：过质量门的只有这些——宁缺毋滥，不凑数`（仅 N<cap 时出现；
  从此「为什么只有 X 篇」在 digest 头自答，无论薄因是源挂还是当天真没料）；③ run-daily.sh
  开跑前探 `pmset -g batt`，电池上跑就在 launchd 日志留 `ON BATTERY` 显眼标记（07-08 早上
  fetch 被睡眠切到 08:42→10:47 的那类慢跑从此一眼自诊；**软件防不了电池合盖睡眠，根治=插电**，
  pmset 08:25 唤醒已装、复核仍在）。测试：test_health_line 扩展 + test_synthesize_thin_delivery_note
  （薄日出现/满日不出现双向）。**分层防线终态：预防（429 退避）→ 自愈（salvage+B2 补课）→
  披露（源失败行+薄推送行+电池标记）**。

## 07-08 迁移日：triage 分块超时 240→480（两机数据定案，同 rerank 7.3 修法）
- **现象**：新机首跑 triage 块 3 撞 240s 超时（重试一发即过）。用户质疑「腾讯电脑没这问题」——
  **数据证伪**：源机 trace 里同款更狠（07-07 ctrm 三连 240s 超时=分块前的全池降级事故；07-08
  hsxr 分块后四块 190/217/197/156s，最重一块距天花板仅 23s）；源机历史 llm 超时 WARN 共 77 条
  （haiku 5 / sonnet 49 / opus 23）。两机块耗时分布一致（新机 173/187/162s）——**不是机器/迁移
  问题，是 240s 默认值对「80 条/块、长 rubric」的常态载荷本就没有余量**，谁先踩只看当块料的长短。
- **修**：`triage.py` 调 `complete_json` 显式 `timeout=480`——deepread(1200)/rerank(480)/
  eval(600/180) 全都早已按各自载荷显式调参，唯 triage 还吃 240 默认；对齐 7.3 rerank 先例。
  断言并入 `test_triage_chunks_with_global_indices`（同 rerank 的 `seen_kw` 手法）。
- **刻意不做**：不改 `claude_code.py` 的 240 默认值（各调用点显式声明自己的载荷预算，比抬全局
  默认更可审计）；不减 CHUNK_SIZE（80 是 07-07 定的平衡点，改它动更多行为）。
- **防线不变**：超时仍会发生（网络/负载波动），靠 retries=3 + 单块启发式降级兜底；本条只是把
  「常态载荷贴着天花板跑」修成「有 2.2× 余量」。

## 07-08 迁移日续：同日多版本（重跑不清史）+ wrangler 部署直连
- **触发**：迁移日两台机器先后投同一天——旧机器晨跑 9 篇（arxiv 饿池）、修复后第二跑 10 篇、
  本机再跑 10 篇。三重暴露：① 本机投递复用 `2026-07-08:list` outTrackId，钉钉**静默忽略新
  cardData**，用户看到的还是 9 行旧卡（deliver_cards.py 文档早已记载此行为，生产路径没接）；
  ② 第二版落盘直接覆盖第一版 items.json/归档 md——**已付 opus token 的产物被清史**；③ 旧机器
  第二跑数据没进迁移包（打包时还在深读中途），永久遗失——只剩钉钉里那张卡。
- **修（radar/core/versioning.py + synthesize/dingtalk_card/_site 三处接线）**：
  「版本」= items.json 的 **id 顺序**（同 id 序=原地更新，regen_v5/rebuild 零版本噪声）；
  变化时旧版改名 `{date}.v{k}.*` + `{date}.versions.json` append 登记（含 **lost 墓碑** =
  投递过但数据不可得，如实展示不装完整）；当前版永远无后缀 → mark/eval/卡片回退零改动；
  卡片 outTrackId 同日 v>1 加 `:v{n}`（parse 只读首段，date 不受影响，nonce 仍可叠加）；
  网页最新页出版本注记、历史版出 `{seg}/v{k}/` 只读子页（同 leak 闸/同能力信封/无投票钮）。
  历史日期零迁移：无登记簿=单版本，首次变化时自动补登 v1。
- **wrangler 直连（web_reader._deploy_env）**：付费代理拖死上传（300s 超时→当跑 web false、
  卡片退 arxiv 链接），直连秒过（手动复部署实测）；deploy 子进程剥 HTTP(S)_PROXY/ALL_PROXY，
  留 `AGENT_RADAR_DEPLOY_PROXY`（如本机 7897）作显式逃生门——镜像 run-serve.sh 的
  AGENT_RADAR_WEB_PROXY 先例。抓源仍走 .env 代理（用户拍板不变）。
- **刻意不做**：不给版本独立 seg（同日同能力信封，`v{k}/` 子路径继承）；不让历史版可投票
  （feedback 锚定 item_id + 内容快照，旧卡投票本就可达；页面投票只指当前版免混淆）；
  不回填 07-08 之前日期的登记簿（无登记簿=单版本语义已覆盖）。
- **07-08 种子**：v1=9 篇（从迁移包冻结快照找回）· v2=10 篇 lost 墓碑（数据未随包，卡片在
  钉钉可人工查看）· v3=10 篇（本机跑）。网页当天页如实展示三版本。

---

# 全面自审（两层，开放式）— 2026-07-08（报告 `docs/SELF_AUDIT_2026-07-07.md`）

CC 独立产出的开放式自审：第一层=实现 vs 文档承诺、第二层=预期本身对不对。方法=3 路只读探索 + 4 域并行执行层审计（正确性/闭环/隐私/运维）+ 1 轮方法论红队 + ~30 只读探针 + 8 题业界联网调研；file:line 主线抽验 10%；零代码改动、零自动应用。

- **核心结论**：管线工程扎实（255 绿、六边形整洁、详解忠实度 93-95%），但**可靠性地基纸糊**（睡眠唤醒防线迁移未重建 + 零备份已丢过数据 + 断供无外部告警）；**三处自我评估失真**侵蚀可信——① 周报逐字承诺"投票→越用越准"而反馈从不进 rerank（文档超卖、闭环空转）；② 断供 0 投递日被健康读数报成"全部正常"、LLM 草案放大成"连续 9 天无掉线"（幸存者偏差）；③ 北极星"选得准"无尺、唯一在测的忠实度已饱和不再产信息。
- **执行层**：11 条动摇使命（RunLock 夺锁并发双投 / 部分投递永久漏报 / 投票超卖 / 选得准无尺 / 幸存者偏差 / 隐私四洞〔CF 历史快照活暴露·git 无强制闸·词表漏关键身份词·leak 闸 fail-open〕/ 睡眠不设防 / 零备份）+ 13 拉低质量 + 12 锦上添花。诚实负结果：时区迁移**不**造成漏报（新鲜度全走 UTC，两份 A1 独立证伪主线初设），只日期标签偏移 + versioning 碰撞。
- **方向层三推翻**：① 固定 10 篇 opus 全量深读 → 自适应分层深读（额度/阅读/测量三重不可持续；smol.ai 同类是分层摘要非固定 N 全深读）；② faithfulness 每日全量当主尺 → 抽样 + 新建"选得准"尺（MiniCheck 降本 + 次日社区信号反查漏检）；③ opus 独占无护栏 → 保留订阅但加硬护栏（动态 N + token 硬闸计入 cache + 连败熔断 + ccusage 监控；deepread 有界 API 兜底逃生门待拍板）。暂缓判定登记簿记 3 个 n 不足命题。
...
# 07-09 ★ 深读 300s 天花板的真根因：两个看门狗，生效值取 min

## 症状
07-09 00:20 那跑 15 次 opus 调用：成功 3、失败 12，**失败全部卡在 303.7±0.2s**。
翻遍 trace 历史（两台机器、五次跑）：**每一次成功的 opus 调用都 <300s**（47.8/91.1/162.5/
196.0/220.5/282.3…），**每一次 >300s 的都死**。一个完美的天花板。
诊断时的两条假路：① 「额度墙」——当日 opus 调用有成功的，证伪；② 「付费代理 300s 上限」
——wrangler 确有此症（07-08 已修），但小调用代理/直连各 6s、长流对照两臂一致，证据不支持。

## 真根因（`strings` 反编译 claude 2.1.205）
```js
function fSi(){ return Math.max(Number(process.env.CLAUDE_STREAM_IDLE_TIMEOUT_MS)||0, 300000) }
function mSi(e){ ... let o=Number(process.env.CLAUDE_BYTE_STREAM_IDLE_TIMEOUT_MS);
                 if(Number.isFinite(o)&&o>0) n=o; ... return Math.min(Math.max(n,1e4),1800000) }
let In = CLAUDE_ENABLE_STREAM_WATCHDOG ?? true,
    Io = fSi(),                                    // 地板 300_000，只认不带 BYTE_ 的名字
    No = Math.min( mSi(En()), In ? Io : Infinity );  // ← 生效期限
```
**有两个看门狗，生效值是 min。** 07-06 的修复只设了 `CLAUDE_BYTE_STREAM_IDLE_TIMEOUT_MS`
（当时的注释还特意写「docs/blogs 流传一个没有 BYTE_ 的错名」）→ `mSi()` 确实升到 1200s，
随即被 `Math.min` 压回 `Io = 300_000`。**修复看起来应用了，其实一天都没生效**——只是那之后
的跑碰巧每篇都 <300s（07-08 本机成功那跑：162–220s），直到 V5 深读遇上真正长的文章。

## 修
`claude_code._run` 同时设 **两个** 名字 + `API_TIMEOUT_MS`，三者都等于本次调用的 timeout，
让 wrapper 成为唯一的期限所有者。`tests/test_llm_timeouts.py` 钉死「两个都设且相等」。

## 教训（比修复本身重要）
- **旋钮设了不等于生效**。反编译时不能只找「我的变量名在不在」，要跟到**它参与的表达式**
  ——这里是一个 `Math.min` 把它吃掉了。
- **没有反例的"修复"不算验证**。07-06 之后没有任何一次 >300s 的调用来证伪它。
- **CLI 没被钉版本**：`AGENT_RADAR_CLAUDE_BIN` 空着，homebrew 在 07-08 23:59 悄悄装了
  2.1.205。这类 env 名/钳制逻辑随时会变 → CLI 升级后必须重跑 `strings` 复核。
- 判据留下：**trace 里若再出现「成功全 <X、失败全 ≈X」的完美天花板，先怀疑某个 X 秒的
  上游期限，而不是额度或网络。**

---

# 07-09 ★★ 更正：上一条「两个看门狗取 min」的根因判断是**错的**

保留上一条不删，因为它的推理过程值得留作反例。**结论错在哪、怎么被推翻的：**

## 被推翻的过程
上一条断言「`No = Math.min(mSi(), Io)` 把 1200s 压回 300s」，于是同时设两个 idle 变量。
**改完重跑，opus 仍在 303.8s 连挂三次。** 假设当场证伪。

## 真根因（三次受控复现，同一条真实 deepread 提示词 76K 字符）
| 条件 | 结果 |
|---|---|
| 两个 idle 旋钮 = 1200000 + `API_TIMEOUT_MS`=1200000，**走代理** | 304s ✗ |
| 同上，**直连**（剥全部代理） | 304s ✗ |
| 同上 + `CLAUDE_ENABLE_BYTE_WATCHDOG=0`（关掉看门狗） | 172s ✗ |
| 同上 + `API_FORCE_IDLE_TIMEOUT=0` | 304s ✗ |
| **换 claude 2.1.201**，什么旋钮都不用 | **267s ✓，13842 字完整详解** |

错误串是 `API Error: Connection closed mid-response.` —— 反编译里这句属于
`network_down / stale_connection` 分支，**不是 `watchdog` 分支**（后者说 "Response stalled
mid-stream"）。看门狗从头到尾就没开过火；我把错误串对错了分支。

## 判据（trace 全历史，最硬的一条）
- 06-30 → 07-07：opus 调用 357s ✓ · 461s ✓ · 529s ✓ · 706s ✓ · **818.9s ✓**（长调用一直正常）
- **07-08 11:34 那跑起**：**≥300s 的调用无一存活**，全部 303.x；<300s 的全部成功。
→ 一个在 07-08 出现的**上游 CLI 回归**。homebrew 在 **07-08 23:59** 把 cask 从 2.1.204
换成 2.1.205；老机器 07-08 上午那跑同样中招（它也在自动升级轨道上）。

## 修
`AGENT_RADAR_CLAUDE_BIN` —— 这个「钉住已知 good 二进制」的钩子**代码里早就写好了，一直空着**。
钉到 npm 装的 2.1.201（`~/.local/share/agent-radar/cc-2.1.201`，`.env` 机器本地）。
`doctor` 新增两条：未钉住 → ⚠；解析到的版本是 2.1.205 → ⚠ 并点名症状。

## 教训（三条，比修复重要）
1. **反编译读到一个「看起来对」的机制，不等于它就是凶手。** 我在没有做「关掉它看看」的
   对照实验前就写进了 commit message。正确顺序是：先证伪，再落笔。
2. **错误串要读到分支。** "Connection closed mid-response" 与 "Response stalled mid-stream"
