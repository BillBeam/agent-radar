...
- **但**：已会主题里的 **全新实证结果 / 反直觉发现 / 新失败模式 / 新机制 / SOTA 突破 / 一手数据 / 新 benchmark·新评测环境·新测量方法（首次让某能力或失败模式变得可度量的那种）** → 仍是「对他新」，**照常按工程价值上浮**——他主场里的真前沿**不许一刀切误杀**。
  （注意：豁免的是"首次打开一种新测量能力"的 benchmark；"又一个同类基准 / 换个数据集重测 / 常规 leaderboard 刷分"不在此列，仍按常规判。）
- `⟨近N天同主题×K⟩`：K 越大 = 这主题最近已反复推过，**再降一档**，除非该条带来上面所说的新结果。
- 例：**降** ——「如何搭一个 agent harness（入门讲解）」「brain/hands 解耦综述」；**保** ——「多步 tool-use RL 训练为何崩溃 + 实测曲线」（已会领域里的新机制 / 新数据）、「首个能测 agent 长程记忆谄媚性的 benchmark」（首次让该失败模式可度量）。
